...
def extract_feature(filename, get_global=get_global):
    ...
    for input_segment in segment_list:
        ...
        if filename.endswith(".txt"):
            ...
            pad_indices = torch.ones(
                (PATCH_LENGTH - input_segment.size(0), PATCH_SIZE)).long() * patchilizer.pad_token_id
        else:
    ...
    if not get_global:
        ...
        if remain_chunk_len == 0:
            ...
            feature_weights = torch.tensor([max_input_length] * full_chunk_cnt + [remain_chunk_len],

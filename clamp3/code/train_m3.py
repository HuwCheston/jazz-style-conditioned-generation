...
def collate_batch(batch):
    ...
    input_patches = torch.nn.utils.rnn.pad_sequence(input_patches, batch_first=True,
                                                    padding_value=patchilizer.pad_token_id)
    ...
    target_patches = torch.nn.utils.rnn.pad_sequence(target_patches, batch_first=True,
                                                     padding_value=patchilizer.pad_token_id)
...
class M3Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_patches = patchilizer.encode(item, add_special_patches=True, truncate=True,
                                            random_truncate=(self.mode == "train"))
...
if __name__ == "__main__":
    ...
    train_set = DataLoader(train_set, batch_size=M3_BATCH_SIZE, collate_fn=collate_batch, sampler=train_sampler,
                           shuffle=(train_sampler is None))
    eval_set = DataLoader(eval_set, batch_size=M3_BATCH_SIZE, collate_fn=collate_batch, sampler=eval_sampler,
                          shuffle=(train_sampler is None))
    ...
    if M3_LOAD_CKPT and os.path.exists(M3_WEIGHTS_PATH):
        ...
        print(
            f"Successfully Loaded Checkpoint from Epoch {checkpoint['epoch']} with loss {checkpoint['min_eval_loss']}")
        checkpoint = None

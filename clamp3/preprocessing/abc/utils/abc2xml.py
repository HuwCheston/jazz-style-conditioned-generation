...
def abc_grammar():  # header, voice and lyrics grammar for ABC
    ...
    grace_notes = Forward()
    chord = Suppress('[') + OneOrMore(chord_note | grace_notes) + Suppress(']') + note_length + Optional(
        tie) + Optional(slur_ends)
    ...
    bare_volta = FollowedBy(
        Literal('[') + Word(nums))  # no barline, but volta follows (volta is parsed in next measure)
    ...
    measure = Group(ZeroOrMore(inline_field) + Optional(bar_left) + ZeroOrMore(element) + bar_right + Optional(
        linebreak) + Optional(lyr_blk))
    noBarMeasure = Group(
        ZeroOrMore(inline_field) + Optional(bar_left) + OneOrMore(element) + Optional(linebreak) + Optional(lyr_blk))
    abc_voice = ZeroOrMore(measure) + Optional(noBarMeasure | Group(bar_left)) + ZeroOrMore(
        inline_field).suppress() + StringEnd()
...
def compChordTab():  # avoid some typing work: returns mapping constant {ABC chordsyms -> musicXML kind}
    ...
    triad = zip('ma Maj maj M mi min m aug dim o + -'.split(),
    ...
    seventh = zip('7 ma7 Maj7 M7 maj7 mi7 min7 m7 dim7 o7 -7 aug7 +7 m7b5 mi7b5'.split(),
                  [dom, maj + ch7, maj + ch7, maj + ch7, maj + ch7, min + ch7, min + ch7, min + ch7, dim + ch7,
                   dim + ch7, min + ch7, aug + ch7, aug + ch7, hd, hd])
    sixth = zip('6 ma6 M6 mi6 min6 m6'.split(), [maj + ch6, maj + ch6, maj + ch6, min + ch6, min + ch6, min + ch6])
    ninth = zip('9 ma9 M9 maj9 Maj9 mi9 min9 m9'.split(),
                [dom + ch9, maj + ch9, maj + ch9, maj + ch9, maj + ch9, min + ch9, min + ch9, min + ch9])
    elevn = zip('11 ma11 M11 maj11 Maj11 mi11 min11 m11'.split(),
                [dom + ch11, maj + ch11, maj + ch11, maj + ch11, maj + ch11, min + ch11, min + ch11, min + ch11])
    thirt = zip('13 ma13 M13 maj13 Maj13 mi13 min13 m13'.split(),
                [dom + ch13, maj + ch13, maj + ch13, maj + ch13, maj + ch13, min + ch13, min + ch13, min + ch13])
...
def splitHeaderVoices(abctext):
    ...
    for x in abctext.splitlines():
        ...
        if x2[:2] == 'W:':
            field = x2[2:].strip()
            ftype = mxm.metaMap.get('W',
                                    'W')  # respect the (user defined --meta) mapping of various ABC fields to XML meta data types
    ...
    if len(hs) == 1:
        header = hs[0]; xs[0] = ''  # no K: present
    else:
        header = hs[0] + hs[1]; xs[0] = ''.join(hs[2:])  # h[1] is the first K:
    ...
    if len(xs) == 1:
        ...
        header += re.sub(r'\[P:.\]', '',
                         xs[0])  # clear all P:-marks from text between K: and first V: and put text in the header
        i = 1
    ...
    for i, x in enumerate(p.objs):  # changing p.objs [i] alters the tree. changing x has no effect on the tree.
        if type(x) == pObj:
            ...
            if x.name == 'grand':  # x.objs contains ordered list of nested parse results within x
                ...
                accept = sum(
                    [1 for nm in nms if nm]) == 1  # accept as grand staff when only one of the voices has a name
                if accept or us[0] == '{*':
                    xs.append(us[1:])  # append voice ids as a list (discard first item '{' or '{*')
                    vdefs = setFristVoiceNameFromGroup(vids, vdefs)
                    p.objs[i] = x.objs[
                        1]  # replace voices by first one in the grand group (this modifies the parse tree)
                else:
...
class MusicXml:
    typeMap = {1: 'long', 2: 'breve', 4: 'whole', 8: 'half', 16: 'quarter', 32: 'eighth', 64: '16th', 128: '32nd',
               256: '64th'}
    dynaMap = {'p': 1, 'pp': 1, 'ppp': 1, 'pppp': 1, 'f': 1, 'ff': 1, 'fff': 1, 'ffff': 1, 'mp': 1, 'mf': 1, 'sfz': 1}
    tempoMap = {'larghissimo': 40, 'moderato': 104, 'adagissimo': 44, 'allegretto': 112, 'lentissimo': 48,
                'allegro': 120, 'largo': 56,
                'vivace': 168, 'adagio': 59, 'vivo': 180, 'lento': 62, 'presto': 192, 'larghetto': 66,
                'allegrissimo': 208, 'adagietto': 76,
                'vivacissimo': 220, 'andante': 88, 'prestissimo': 240, 'andantino': 96}
    wedgeMap = {'>(': 1, '>)': 1, '<(': 1, '<)': 1, 'crescendo(': 1, 'crescendo)': 1, 'diminuendo(': 1,
                'diminuendo)': 1}
    ...
    ornMap = {'trill': 'trill-mark', 'T': 'trill-mark', 'turn': 'turn', 'uppermordent': 'inverted-mordent',
              'lowermordent': 'mordent',
              'pralltriller': 'inverted-mordent', 'mordent': 'mordent', 'turn': 'turn', 'invertedturn': 'inverted-turn'}
    tecMap = {'upbow': 'up-bow', 'downbow': 'down-bow', 'plus': 'stopped', 'open': 'open-string',
              'snap': 'snap-pizzicato',
              'thumb': 'thumb-position'}
    capoMap = {'fine': ('Fine', 'fine', 'yes'), 'D.S.': ('D.S.', 'dalsegno', 'segno'),
               'D.C.': ('D.C.', 'dacapo', 'yes'), 'dacapo': ('D.C.', 'dacapo', 'yes'),
               'dacoda': ('To Coda', 'tocoda', 'coda'), 'coda': ('coda', 'coda', 'coda'),
    ...
    modTab = {'maj': 'major', 'm': 'minor', 'min': 'minor', 'mix': 'mixolydian', 'dor': 'dorian', 'phr': 'phrygian',
              'lyd': 'lydian', 'loc': 'locrian'}
    clefMap = {'alto1': ('C', '1'), 'alto2': ('C', '2'), 'alto': ('C', '3'), 'alto4': ('C', '4'), 'tenor': ('C', '4'),
               'bass3': ('F', '3'), 'bass': ('F', '4'), 'treble': ('G', '2'), 'perc': ('percussion', ''),
               'none': ('', ''), 'tab': ('TAB', '5')}
    ...
    metaTab = {'O': 'origin', 'A': 'area', 'Z': 'transcription', 'N': 'notes', 'G': 'group', 'H': 'history',
               'R': 'rhythm',
               'B': 'book', 'D': 'discography', 'F': 'fileurl', 'S': 'source', 'P': 'partmap', 'W': 'lyrics'}
    metaMap = {'C': 'composer'}  # mapping of composer is fixed
    metaTypes = {'composer': 1, 'lyricist': 1, 'poet': 1, 'arranger': 1, 'translator': 1,
                 'rights': 1}  # valid MusicXML meta data types
    ...
    def mkPitch(s, acc, note, oct, lev):
        if s.percVoice:  # percussion map switched off by perc=off (see doClef)
            ...
            if not tup:  # add percussion map for unmapped notes in this part
                midi = str(
                    octnum * 12 + [0, 2, 4, 5, 7, 9, 11]['CDEFGAB'.index(step.upper())] + {'^': 1, '_': -1}.get(acc,
                                                                                                                0) + 12)
                notehead = {'^': 'x', '_': 'circle-x'}.get(acc, 'normal')
                if s.pMapFound: info('no I:percmap for: %s%s in part %s, voice %s' % (
                acc + note, -oct * ',' if oct < 0 else oct * "'", s.pid, s.vid))
    ...
    def mkNote(s, n, lev):
        ...
        if getattr(n, 'tie', 0):
            ...
            addElem(nt, tie, lev + 1)
        if (s.midprg != ['', '', '',
                         ''] or midi) and n.name != 'rest':  # only add when %%midi was present or percussion
            ...
            if instId not in s.midiInst: s.midiInst[instId] = (
            s.pid, s.vid, chan, midi, s.midprg[2], s.midprg[3])  # for instrument list in mkScorePart
    ...
    def doNotations(s, n, decos, ptup, alter, tupnotation, tstop, nt, lev):
        ...
        for pt, (tie_alter, nts, vnum, ntelm) in sorted(
                list(s.ties.items())):  # scan all open ties and delete illegal ones
        ...
        if tstart:  # start a tie
            s.ties[ptup] = (
            alter, nots, s.overlayVnum, nt)  # remember pitch tuple to stop tie and apply same alteration
        ...
        if decos:  # look for slurs and decorations
            ...
            for d in decos:  # do all slurs and decos
                if d in slurMap:
                    s.slurbeg.append(d); continue  # slurs made in while loop at the end
                    ...
                    if d[1] == '(':
                        tp = 'start'; s.glisnum += 1; gn = s.glisnum
                    else:
                        tp = 'stop'; gn = s.glisnum; s.glisnum -= 1
            ...
            if lyrobj.name == 'syl':
                ...
                if dash:
                    if i in s.lyrdash:
                        ...
                        type = 'begin'; s.lyrdash[i] = 1
                else:
                    if i in s.lyrdash:
                        type = 'end';   del s.lyrdash[i]
                    else:
                        type = 'single'
                ...
                info('lyric extend error'); continue
            else:
    ...
    def doFields(s, maat, fieldmap, lev):
        ...
        def doClef(field):
            ...
            if clef == 'tab':
                ...
                s.tunmid = [int(boct) * 12 + [0, 2, 4, 5, 7, 9, 11]['CDEFGAB'.index(bstep)] + 12 + s.capo for
                            bstep, boct in s.tuning]
        ...
        for ftype, field in fieldmap.items():
            ...
            if ftype == 'Div':  # not an abc field, but handled as if
                ...
                try:
                    s.mdur = simplify(eval(beats),
                                      int(btype))  # measure duration for Z and X rests (eval allows M:2+3/4)
                except:
    ...
    def doTempo(s, maat, field, lev):
        ...
        if t:
            try:
                ...
                info('conversion error: %s' % field); return
    ...
    def mkPartlist(s, vids, partAttr, lev):
        ...
        for g in (s.groups or vids):  # brace/bracket or abc_voice_id
            if g == '[':
                g_num += 1; addPartGroup('bracket', g_num)
            elif g == '{':
                g_num += 1; addPartGroup('brace', g_num)
            elif g in '}]':
    ...
    def doField_I(s, type, x, instDir, addTrans):
        ...
        def readPercMap(x):  # parse I:percmap <abc_note> <step> <MIDI> <notehead>
            ...
            def midiVal(acc, step, oct):  # abc note -> midi note number
                ...
                return oct * 12 + [0, 2, 4, 5, 7, 9, 11]['CDEFGAB'.index(step.upper())] + {'^': 1, '_': -1, '=': 0}.get(
                    acc, 0) + 12
            ...
            if r1:
                ...
                if sign:  # relative staff number
                    ...
                    try:
                        ...
                        vabc = 0; info('abc staff %s does not exist' % num)
    ...
    def voiceNamesAndMaps(s, ps):  # get voice names and mappings
        ...
        for vid, vcedef, vce in ps:  # vcedef == emtpy or first pObj == voice definition
            ...
            s.staveDefs += [x.replace('%5d', ']') for x in xs if
                            x.startswith('score') or x.startswith('staves')]  # filter %%score and %%staves
        return vdefs
    ...
    def doHeaderField(s, fld, attrmap):
        ...
        if type == 'M':
            ...
            type = s.metaMap.get(type,
                                 type)  # respect the (user defined --meta) mapping of various ABC fields to XML meta data types
    ...
    def parse(s, abc_string, rOpt=False, bOpt=False, fOpt=False):
        ...
        try:
            ...
            for id, voice in voices:
                if lbrk_insert:  # insert linebreak at EOL
                    ...
                    voice = '\n'.join(
                        [balk.rstrip('$!') + '$' if has_abc(balk) else balk for balk in voice.splitlines()])
                prevLeftBar = None  # previous voice ended with a left-bar symbol (double repeat)
                s.orderChords = s.fOpt and (
                            'tab' in voice[:200] or [x for x in hs if x.t[0] == 'K' and 'tab' in x.t[1]])
        ...
        for vid, _, vce in ps:  # voice id, voice parse tree
            ...
            if s.midprg != ['', '', '', ''] and not s.percVoice:  # when a part has only rests
                instId = 'I%s-%s' % (s.pid, s.vid)
                if instId not in s.midiInst: s.midiInst[instId] = (
                s.pid, s.vid, s.midprg[0], s.midprg[1], s.midprg[2], s.midprg[3])
...
def convert(pad, fnm, abc_string, mxl, rOpt=False, tOpt=False, bOpt=False,
            fOpt=False):  # not used, backwards compatibility
...
if __name__ == '__main__':
    ...
    parser.add_option("-m", action="store", help="skip SKIP (0) tunes, then read at most NUM (1) tunes", nargs=2,
                      type='int', default=(0, 1), metavar='SKIP NUM')
    ...
    parser.add_option("-z", "--mxl", dest="mxl", help="store as compressed mxl, MODE = a(dd) or r(eplace)", default='',
                      metavar='MODE')
    ...
    parser.add_option("--meta", action="store", help="map infofields to XML metadata, MAP = R:poet,Z:lyricist,N:...",
                      default='', metavar='MAP')

...
class MusicHubertModel(HubertModel):
    ...
    def __init__(
        ...
        self.feature_projection = MusicHubertFeatureProjection(
            config)  # replace Feature Projection for introcuing new feature

        if self.config.feature_extractor_cqt:
            print('initializing cqt extractor for MusicHubert')
            self.feature_extractor_cqt = nnAudioFeatures.cqt.CQT(sr=self.config.sample_rate,
                                                                 hop_length=self.config.sample_rate // 50, fmin=32.7,
                                                                 fmax=None,

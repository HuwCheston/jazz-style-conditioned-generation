...
"""Relative Position Representation modules, taken from https://github.com/gwinndr/MusicTransformer-Pytorch"""
...
class TransformerEncoderRPR(torch.nn.Module):
...
class TransformerEncoderLayerRPR(torch.nn.Module):
    """For Relative Position Representation support (https://arxiv.org/abs/1803.02155)"""
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, er_len=None):
        ...
        self.linear1 = nn.modules.Linear(d_model, dim_feedforward)
        self.dropout = nn.modules.Dropout(dropout)
        self.linear2 = nn.modules.Linear(dim_feedforward, d_model)

        self.norm1 = nn.modules.LayerNorm(d_model)
        self.norm2 = nn.modules.LayerNorm(d_model)
        self.dropout1 = nn.modules.Dropout(dropout)
        self.dropout2 = nn.modules.Dropout(dropout)
...
class MultiheadAttentionRPR(torch.nn.Module):
    """For Relative Position Representation support (https://arxiv.org/abs/1803.02155)"""
    ...
    def __init__(
        ...
        self.out_proj = nn.modules.Linear(embed_dim, embed_dim, bias=bias)
...
def multi_head_attention_forward_rpr(
        ...
        static_k: torch.tensor = None,
        static_v: torch.tensor = None,
        rpr_mat: torch.tensor = None
) -> tuple:
...
def _get_valid_embedding(er, len_q, *_):
